...
import { CacheOnlyPlaceProvider } from "./core/place-resolution.js";
...
type Command="init"|"ingest"|"dates"|"places"|"kinship"|"presence"|"candidates"|"score"|"export"|"all";const COMMANDS:readonly Command[]=["init","ingest","dates","places","kinship","presence","candidates","score","export","all"];
interface Options{command:Command;dbPath:string;configPath?:string;gedcomPath?:string;verbose:boolean}
function parseArgs(argv:readonly string[]):Options{const command=argv[0] as Command;if(!COMMANDS.includes(command))throw new Error(`Usage: samesoil <${COMMANDS.join("|")}> [--db path] [--config path] [--gedcom path] [--verbose]`);let dbPath="samesoil.sqlite",configPath:string|undefined,gedcomPath:string|undefined,verbose=false;for(let index=1;index<argv.length;index++){const arg=argv[index];if(arg==="--verbose")verbose=true;else if(arg==="--db"||arg==="--config"||arg==="--gedcom"){const value=argv[index+1];if(value===undefined)throw new Error(`${arg} requires a path`);if(arg==="--db")dbPath=value;else if(arg==="--config")configPath=value;else gedcomPath=value;index++}else throw new Error(`Unknown argument: ${arg}`)}return{command,dbPath:resolve(dbPath),verbose,...(configPath===undefined?{}:{configPath:resolve(configPath)}),...(gedcomPath===undefined?{}:{gedcomPath:resolve(gedcomPath)})}}
export async function main(argv:readonly string[]=process.argv.slice(2)):Promise<void>{const options=parseArgs(argv),files=new NodeFileSourceAdapter(),env=new NodeEnvAdapter(),root=process.cwd(),defaults=await files.readText(resolve(root,"config/default.json")),override=options.configPath===undefined?undefined:await files.readText(options.configPath),loaded=await loadConfig(defaults,override,env),storage=new NodeStorageAdapter(options.dbPath);try{const ctx:EngineContext={storage,fileSource:files,artifactSink:new NodeArtifactSinkAdapter(resolve(options.dbPath,"../artifacts")),progress:new ConsoleProgressReporter(options.verbose),env,placeProvider:new CacheOnlyPlaceProvider(storage,env),config:loaded.config,configHash:loaded.hash,...(options.gedcomPath===undefined?{}:{inputPath:options.gedcomPath})};if(options.command==="init")migrate(storage,await files.readText(resolve(root,"src/db/schema.sql")));else if(options.command==="ingest")await ingest(ctx);else if(options.command==="dates")await dates(ctx);else if(options.command==="places")await places(ctx);else if(options.command==="all"){await ingest(ctx);await dates(ctx);await places(ctx)}console.log(`command=${options.command}`);console.log(`db=${options.dbPath}`);console.log(`config_hash=${loaded.hash}`)}finally{storage.close()}}
