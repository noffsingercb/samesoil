...
//
// Column names below are taken verbatim from src/db/schema.sql:
// scored_candidates.candidate_id references candidate_pairs.id (there is no
// pair_id column); scored_candidates stores score/s_temporal/s_proximity/
// s_precision/s_independence/s_unrelatedness, not total_score/component_*;
// candidate_pairs links to presence_intervals via a_interval_id/b_interval_id
// (there is no presence_a_id/presence_b_id) and already carries a_id/b_id for
// the individuals join; individuals.name_full (not full_name); places has
// lat/lon/place_normalized (not normalized) and is reached from
// presence_intervals.place_id, since events has no place_id column.
// ---------------------------------------------------------------------------
const EXPORT_QUERY = `
SELECT
  sc.candidate_id            AS candidate_id,
  sc.score                   AS total_score,
  sc.s_temporal               AS component_temporal,
  sc.s_proximity              AS component_spatial,
  sc.s_precision              AS component_place_precision,
  sc.s_independence           AS component_source_independence,
  sc.s_unrelatedness          AS component_unrelatedness,
  ...
  ia.name_full                 AS individual_a_name,
  ea.event_type                AS event_a_type,
  ea.date_raw                  AS event_a_date_raw,
  pa.place_raw                 AS place_a_raw,
  pa.place_normalized          AS place_a_resolved,
  pa.precision_tier            AS place_a_tier,
  pa.radius_km                 AS place_a_radius_km,
  pa.lat                       AS place_a_lat,
  pa.lon                       AS place_a_lon,

  ib.id                        AS individual_b_id,
  ib.name_full                 AS individual_b_name,
  eb.event_type                AS event_b_type,
  eb.date_raw                  AS event_b_date_raw,
  pb.place_raw                 AS place_b_raw,
  pb.place_normalized          AS place_b_resolved,
  pb.precision_tier            AS place_b_tier,
  pb.radius_km                 AS place_b_radius_km,
  pb.lat                       AS place_b_lat,
  pb.lon                       AS place_b_lon

FROM scored_candidates sc
JOIN candidate_pairs cp ON cp.id = sc.candidate_id
JOIN presence_intervals pia ON pia.id = cp.a_interval_id
JOIN presence_intervals pib ON pib.id = cp.b_interval_id
JOIN individuals ia ON ia.id = cp.a_id
JOIN individuals ib ON ib.id = cp.b_id
JOIN events ea ON ea.id = pia.event_id
JOIN events eb ON eb.id = pib.event_id
JOIN places pa ON pa.id = pia.place_id
JOIN places pb ON pb.id = pib.place_id
WHERE (ia.is_living = 0 OR ? = 1)
  AND (ib.is_living = 0 OR ? = 1)
ORDER BY sc.score DESC, sc.candidate_id ASC
`
...
//
// places has no direct link to events (events has no place_id column);
// presence_intervals is the join table carrying place_id, event_id, and
// individual_id together, so it alone is sufficient here.
// ---------------------------------------------------------------------------
...
WITH affected AS (
  SELECT
    ...
    p.geocode_conf AS confidence,
    p.review_status AS review_status,
    pi.event_id AS event_id,
    pi.individual_id AS individual_id
  FROM places p
  JOIN presence_intervals pi ON pi.place_id = p.id
...
function renderBranchIntersections(
	...
	for (const r of rows) {
		...
		const decade = decadeOf((r.overlap_start as string) ?? null)
		const [b1, b2] = [branchA, branchB].sort() as [string, string]
