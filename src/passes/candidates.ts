...
import { decades, haversineKm, temporalMatch, type TemporalRelation } from "../core/candidates.js";
...
interface Draft { readonly aIntervalId: number; readonly bIntervalId: number; readonly aId: number; readonly bId: number; readonly overlapStart: string; readonly overlapEnd: string; readonly overlapDays: number; readonly temporalRelation: TemporalRelation; readonly temporalGapDays: number; readonly distanceKm: number; readonly combinedRadius: number }
const KM_PER_DEGREE = 111.32; const TIER_ORDER = ["address", "locality", "district", "region", "country"] as const;
...
export async function run(ctx: EngineContext): Promise<void> {
  ...
  const drafts: Draft[] = []; let comparisons = 0, temporalRejections = 0, distanceRejections = 0, kinshipQueries = 0, peakIntervalNeighbors = 0, overlapMatches = 0, nearMatches = 0; const kinship = createKinshipService(ctx);
  ...
  for (const [index, row] of rows.entries()) {
    ...
    for (const rowDecade of decades(row.date_start, row.date_end)) for (let decadeOffset = -1; decadeOffset <= 1; decadeOffset += 1) for (let dx = -longitudeNeighbors; dx <= longitudeNeighbors; dx += 1) for (let dy = -1; dy <= 1; dy += 1) for (const other of buckets.get(bucketKey(rowDecade + decadeOffset, x + dx, y + dy)) ?? []) {
      if (other.id <= row.id || seenOtherIntervals.has(other.id)) continue; seenOtherIntervals.add(other.id); comparisons += 1;
      const temporal = temporalMatch({ start: row.date_start, end: row.date_end }, { start: other.date_start, end: other.date_end }, ctx.config.scoring.temporal_candidate_window_days); if (temporal === null) { temporalRejections += 1; continue; }
      ...
      kinship.resolve(row.individual_id, other.individual_id); kinshipQueries += 1; if (temporal.relation === "overlap") overlapMatches += 1; else nearMatches += 1;
      drafts.push({ aIntervalId: row.id, bIntervalId: other.id, aId: row.individual_id, bId: other.individual_id, overlapStart: temporal.start, overlapEnd: temporal.end, overlapDays: temporal.overlapDays, temporalRelation: temporal.relation, temporalGapDays: temporal.gapDays, distanceKm: distance, combinedRadius });
    }
  ...
  ctx.storage.transaction((): void => { for (const row of drafts) ctx.storage.run("INSERT INTO candidate_pairs(a_interval_id,b_interval_id,a_id,b_id,overlap_start,overlap_end,overlap_days,temporal_relation,temporal_gap_days,distance_km,combined_radius) VALUES(?,?,?,?,?,?,?,?,?,?,?)", [row.aIntervalId, row.bIntervalId, row.aId, row.bId, row.overlapStart, row.overlapEnd, row.overlapDays, row.temporalRelation, row.temporalGapDays, row.distanceKm, row.combinedRadius]); });
  ctx.progress.passFinished(pass, { rows_in: inputRows.length, eligible_rows: rows.length, rows_out: drafts.length, overlap_matches: overlapMatches, near_matches: nearMatches, excluded_coarse_place: excludedCoarsePlace, excluded_low_confidence: excludedLowConfidence, blocked_comparisons: comparisons, rejected_temporal: temporalRejections, rejected_distance: distanceRejections, kinship_queries: kinshipQueries, peak_interval_neighbors: peakIntervalNeighbors, elapsed_ms: elapsed(started, ctx.env.now()) });
}
