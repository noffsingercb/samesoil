...
const TABLES = ["scored_candidates", "suppression_log", "candidate_pairs", "presence_intervals", "kinship_distance", "kinship_edges", "event_dates", "events", "family_children", "families", "individuals", "raw_records", "places", "geocode_cache"];
export function migrate(storage, schemaSql) {
    storage.applySchema("CREATE TABLE IF NOT EXISTS meta (key TEXT PRIMARY KEY, value TEXT NOT NULL);");
    const current = storage.get("SELECT value FROM meta WHERE key = ?", ["schema_version"]);
...
export function rebuildDerived(storage, schemaSql) {
    ...
    try {
        storage.transaction(() => {
            ...
            for (const table of TABLES)
                storage.applySchema(`DROP TABLE IF EXISTS ${table};`);
            storage.applySchema(schemaSql);
