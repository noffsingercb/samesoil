...
try {
  ...
  console.table(db.prepare(`SELECT sc.candidate_id,ROUND(sc.score,6) AS score,ia.name_full AS person_a,ib.name_full AS person_b,ea.event_type AS event_a,eb.event_type AS event_b,ea.date_raw AS date_a,eb.date_raw AS date_b,ea.place_raw AS place_a,eb.place_raw AS place_b,c.overlap_start,c.overlap_end,ROUND(c.distance_km,2) AS distance_km FROM scored_candidates sc JOIN candidate_pairs c ON c.id=sc.candidate_id JOIN presence_intervals pa ON pa.id=c.a_interval_id JOIN presence_intervals pb ON pb.id=c.b_interval_id JOIN individuals ia ON ia.id=c.a_id JOIN individuals ib ON ib.id=c.b_id JOIN events ea ON ea.id=pa.event_id JOIN events eb ON eb.id=pb.event_id ORDER BY sc.score DESC,sc.candidate_id LIMIT 30`).all());
  console.log("TOP CANDIDATE EXPLANATIONS");
  for (const row of db.prepare(`SELECT candidate_id,ROUND(score,6) AS score,explanation FROM scored_candidates ORDER BY score DESC,candidate_id LIMIT 30`).all()) console.log(`[${row.candidate_id}] score=${row.score} ${row.explanation}`);
